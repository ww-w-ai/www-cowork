...
일반 기능 작업은 Goal을 자동 생성하지 않습니다. Goal은 사용자가 opt-in 단어 **`goal`**을 말했을 때만 만들어집니다 — 턴과 컴팩션을 넘어 살아남는 구조적·대량·배리어형 실행을 여는 단어입니다. (Claude Code에서 같은 자리의 단어는 `ultracode`이고, Goal이 아니라 `Workflow`를 엽니다. 두 단어는 서로 대체되지 않습니다.) 기존 Goal을 이어갈 때는 Goal의 thread ID로 정확한 세션을 복원하며, 단순히 최신 transcript를 추측하지 않습니다.
